...
function artifactsFor(stack: Stack, size: number, r: () => number, path: string): CleanupArtifact[] {
  ...
  switch (stack) {
    case "node":
      out.push(mk("node_modules/", "node_modules", "nodeDependencies", size * (0.6 + r() * 0.3), "safe", "Installed packages generated from the lockfile.", "pnpm install --frozen-lockfile"));
...
function makeProjects(root: string, now: number): Project[] {
  ...
  return NAMES.map(([name, stack], i) => {
    ...
    const artifacts = artifactsFor(stack, size, r, path);
    ...
    return {
      ...
      stacks: [stack],
      frameworks,
      packageManager: stack === "node" ? (["pnpm", "npm", "yarn"][i % 3] ?? "npm") : stack === "python" ? "pip" : null,
